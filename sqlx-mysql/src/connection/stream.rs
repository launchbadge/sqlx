...
impl<S: Socket> MySqlStream<S> {
    ...
    pub fn maybe_enable_compression(self, options: &MySqlConnectOptions) -> Self {
        let compression_stream = match options.compression {
            Some(c) if c.is_supported(&self.capabilities) => {
                CompressionMySqlStream::compressed(self.compression_stream.socket, c)
            }
            _ => CompressionMySqlStream::not_compressed(self.compression_stream.socket),
        };
        MySqlStream {
            compression_stream,
            server_version: self.server_version,
