...
pub struct CompressionConfig(pub(crate) Compression, pub(crate) u8);
impl CompressionConfig {
    pub(crate) fn is_supported(&self, capabilities: &Capabilities) -> bool {
        match self {
            CompressionConfig(Compression::ZLIB, _) => {
                capabilities.contains(Capabilities::COMPRESS)
            }
            CompressionConfig(Compression::ZSTD, _) => {
...
impl Compression {
    ...
    /// If the level is out of range, an [`Error::InvalidArgument`] is returned.
    ///
    /// # Returns
    ///
