...
use crate::postgres::{PgArguments, PgTypeInfo, PgTypeKind, PgValueFormat, PgValueRef, Postgres};
use bitflags::_core::str::from_utf8;
use bytes::Buf;

#[doc(hidden)]
pub struct PgRecordEncoder<'a> {
    buf: &'a mut PgArguments,
    off: usize,
    num: u32,
}
...
impl<'a> PgRecordEncoder<'a> {
    #[doc(hidden)]
    pub fn new(buf: &'a mut PgArguments) -> Self {
        let off = buf.len();
