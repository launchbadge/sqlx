...
{
    #[inline]
    fn encode_by_ref(&self, buf: &mut PgArguments) -> IsNull {
    ...
    fn encode_by_ref(&self, buf: &mut PgArguments) -> IsNull {
        ...
        match T::type_info().0 {
            PgType::DeclareWithName(name) => {
                // FIXME: Handle no type ID known here
                // self.fetch_type_id_by_name(name).await?
                todo!("user-defined types unsupported in arrays");
            }
