...
use crate::mysql::{MySql, MySqlArguments, MySqlValueFormat, MySqlValueRef};
...
fn encode_date(date: &Date, buf: &mut MySqlArguments) {
...
fn encode_time(time: &Time, include_micros: bool, buf: &mut MySqlArguments) {
    buf.push(time.hour());
