...
use super::DatabaseMacroInput;

pub fn expand_database_macros(input: DatabaseMacroInput) -> TokenStream {
    let env = input.env;
    ...
    macro_rules! query (
        ...
        ($query:expr) => ({
            sqlx::sqlx_macros::expand_query!(source = $query, db_url_env = #env)
        });
        ...
        ($query:expr, $($args:tt)*) => ({
            sqlx::sqlx_macros::expand_query!(source = $query, args = [$($args)*], db_url_env = #env)
        })
    );
    pub(crate) use query;
    ...
    macro_rules! query_unchecked (
        ($query:expr) => ({
            sqlx::sqlx_macros::expand_query!(source = $query, checked = false, db_url_env = #env)
        });
        ($query:expr, $($args:tt)*) => ({
            sqlx::sqlx_macros::expand_query!(source = $query, args = [$($args)*], checked = false, db_url_env = #env)
        })
    );
    ...
    macro_rules! query_file (
        ($path:literal) => ({
            sqlx::sqlx_macros::expand_query!(source_file = $path, db_url_env = #env)
        });
        ($path:literal, $($args:tt)*) => ({
            sqlx::sqlx_macros::expand_query!(source_file = $path, args = [$($args)*], db_url_env = #env)
        })
    );
    ...
    macro_rules! query_file_unchecked (
        ($path:literal) => ({
            sqlx::sqlx_macros::expand_query!(source_file = $path, checked = false, db_url_env = #env)
        });
        ($path:literal, $($args:tt)*) => ({
            sqlx::sqlx_macros::expand_query!(source_file = $path, args = [$($args)*], checked = false, db_url_env = #env)
        })
    );
    pub(crate) use query_file_unchecked;
    ...
    macro_rules! query_as (
        ($out_struct:path, $query:expr) => ( {
            sqlx::sqlx_macros::expand_query!(record = $out_struct, source = $query, db_url_env = #env)
        });
        ($out_struct:path, $query:expr, $($args:tt)*) => ( {
            sqlx::sqlx_macros::expand_query!(record = $out_struct, source = $query, args = [$($args)*], db_url_env = #env)
        })
    );
    ...
    macro_rules! query_file_as (
        ($out_struct:path, $path:literal) => ( {
            sqlx::sqlx_macros::expand_query!(record = $out_struct, source_file = $path, db_url_env = #env)
        });
        ($out_struct:path, $path:literal, $($args:tt)*) => ( {
            sqlx::sqlx_macros::expand_query!(record = $out_struct, source_file = $path, args = [$($args)*], db_url_env = #env)
        })
    );
    ...
    macro_rules! query_as_unchecked (
        ($out_struct:path, $query:expr) => ( {
            sqlx::sqlx_macros::expand_query!(record = $out_struct, source = $query, checked = false, db_url_env = #env)
        });

        ($out_struct:path, $query:expr, $($args:tt)*) => ( {
            sqlx::sqlx_macros::expand_query!(record = $out_struct, source = $query, args = [$($args)*], checked = false, db_url_env = #env)
        })
    );
    pub(crate) use query_as_unchecked;
    ...
    macro_rules! query_file_as_unchecked (
        ($out_struct:path, $path:literal) => ( {
            sqlx::sqlx_macros::expand_query!(record = $out_struct, source_file = $path, checked = false, db_url_env = #env)
        });

        ($out_struct:path, $path:literal, $($args:tt)*) => ( {
            sqlx::sqlx_macros::expand_query!(record = $out_struct, source_file = $path, args = [$($args)*], checked = false, db_url_env = #env)
        })
    );
    pub(crate) use query_file_as_unchecked;
    ...
    macro_rules! query_scalar (
        ($query:expr) => (
            sqlx::sqlx_macros::expand_query!(scalar = _, source = $query, db_url_env = #env)
        );
        ($query:expr, $($args:tt)*) => (
            sqlx::sqlx_macros::expand_query!(scalar = _, source = $query, args = [$($args)*], db_url_env = #env)
        )
    );
    pub(crate) use query_scalar;
    ...
    macro_rules! query_file_scalar (
        ($path:literal) => (
            sqlx::sqlx_macros::expand_query!(scalar = _, source_file = $path, db_url_env = #env)
        );
        ($path:literal, $($args:tt)*) => (
            sqlx::sqlx_macros::expand_query!(scalar = _, source_file = $path, args = [$($args)*], db_url_env = #env)
        )
    );
    pub(crate) use query_file_scalar;
    ...
    macro_rules! query_scalar_unchecked (
        ($query:expr) => (
            sqlx::sqlx_macros::expand_query!(scalar = _, source = $query, checked = false, db_url_env = #env)
        );
        ($query:expr, $($args:tt)*) => (
            sqlx::sqlx_macros::expand_query!(scalar = _, source = $query, args = [$($args)*], checked = false, db_url_env = #env)
        )
    );
    pub(crate) use query_scalar_unchecked;
    ...
    macro_rules! query_file_scalar_unchecked (
        ($path:literal) => (
            sqlx::sqlx_macros::expand_query!(scalar = _, source_file = $path, checked = false, db_url_env = #env)
        );
        ($path:literal, $($args:tt)*) => (
            sqlx::sqlx_macros::expand_query!(scalar = _, source_file = $path, args = [$($args)*], checked = false, db_url_env = #env)
        )
    );
    pub(crate) use query_file_scalar_unchecked;
