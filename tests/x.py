...
def clean_profile_files():
    command = 'find tests/.cache/target/debug/deps/ -name "*.gc*" | xargs rm -rf'
    subprocess.run(
        ["docker-compose", "run", "--rm", "sqlx", "bash", "-c", command],
...
def run(command, comment=None, env=None, service=None, tag=None):
    ...
    if argv.test:
        args.extend(["--test", argv.test])

    if unknown:
        args.extend(["--", *unknown])

    print(f"\x1b[93m $ {command} {' '.join(args)}\x1b[0m")

    subprocess.run(
        ["docker-compose", "run", "--rm", *environ, "sqlx", *command.split(" "), *args],
        cwd=os.path.dirname(__file__),
    )
...
run("cargo c", comment="check with a default set of features")

run(
    "cargo c --no-default-features --features runtime-async-std,mysql,postgres,sqlite,all-types",
    comment="check with async-std",
)

run(
    "cargo c --no-default-features --features runtime-tokio,mysql,postgres,sqlite,all-types",
    comment="check with tokio",
)

run(
    "cargo c --no-default-features --features runtime-actix,mysql,postgres,sqlite,all-types",
